	...
	<body>
		...
		<div id="show-musee" class="content scaffold-show" role="main">
			...
			<ol class="property-list musee">
				...
				<g:if test="${museeInstance?.demandevisites}">
				<li class="fieldcontain">
					<span id="demandevisites-label" class="property-label"><g:message code="musee.demandevisites.label" default="Demandevisites" /></span>
					
						<g:each in="${museeInstance.demandevisites}" var="d">
						<span class="property-value" aria-labelledby="demandevisites-label"><g:link controller="demandeVisite" action="show" id="${d.id}">${d?.encodeAsHTML()}</g:link></span>
						</g:each>
